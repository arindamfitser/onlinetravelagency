...
class AjaxController extends Controller{
  ...
  public function destinationFetchRoom(Request $request){
    ...
    // session()->forget('t_start');
    // session()->forget('t_end');
    // session()->forget('keywords');
    // session()->forget('quantity_adults');
    // session()->forget('quantity_child');
    // session()->forget('ab');
    // session()->forget('rooms');
    // session()->forget('num_room');
    // session()->forget('quantityRooms');
    // session()->forget('noguests');
    // session()->forget('totalNight');
    // session()->save();
    // Session::forget('t_start');
    // Session::forget('t_end');
    // Session::forget('keywords');
    ...
    if($request->roomHotelToken == NULL || empty($request->roomHotelToken)):
      ...
      foreach($request->norm as $keyyy => $val):
        ...
        $totalRoom++;
      endforeach;
      Session::put('t_start',$cIn);
      Session::put('t_end', $cOut);
      Session::put('keywords', $request->roomSearchKeyword);
      Session::put('quantity_adults', $totalAdults);
      Session::put('quantity_child', $totalKids);
      Session::put('ab', $ab);
      ...
      Session::put('quantityRooms', $totalRoom);
      Session::put('noguests', ($totalAdults + $totalKids));
      Session::put('totalNight', $nights);
      Session::save();

      // session(['ab'               => $ab]);    
      // session(['rooms'            => $ab]);
      // session(['num_room'         => $totalRoom]);
      // session(['quantityRooms'    => $totalRoom]);
      // session(['keywords'         => $request->roomSearchKeyword]);
      // session(['t_start'          => $cIn]);
      // session(['t_end'            => $cOut]);
      // session(['quantity_adults'  => $totalAdults]);
      // session(['quantity_child'   => $totalKids]);
      // session(['noguests'         => ($totalAdults + $totalKids)]);
      // session(['totalNight'       => $nights]);
      // session()->save();
      ...
      if(!empty($finalSearchRooms)):
        $html = view('frontend.hotels.ajaxHotel', compact('finalSearchRooms'))->render();
      endif;
    endif;
