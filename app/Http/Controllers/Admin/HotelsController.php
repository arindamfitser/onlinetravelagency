...
class HotelsController extends Controller{
    ...
    public function uploadcsv(Request $request){
        ...
        if($request->file('upload_csv')){
            if($request->file('upload_csv')->extension() == 'csv'):
                ...
                $msg = 'Hotels added successfully!';
                unlink($path);
            else:
                $msg = 'csv format file only!';
            endif;
        }
    ...
    public function update(Request $request, $lang, $id){
        ...
        switch ($form_no) {
          case 1:
            ...
            $hotels->stuba_id = $request->stuba_id;
    ...
    public function direct_contact(){
        $hotels = Hotels::select('*', 'hotels.id as h_id')->leftJoin('fishermen', 'hotels.id', '=', 'fishermen.hotel_id')->leftJoin('hotel_contacts', 'hotels.id', '=', 'hotel_contacts.hotel_id')->orderBy('hotels.id', 'ASC')->get()->all();
        return view('admin.hotels.direct_contact', compact('hotels'));
    }
    public function editDirectContact(Request $request){
        DB::table($request->table)
              ->where($request->updateKey, $request->hotel_id)
              ->update([$request->key => $request->changedData]);
        if($request->key == 'state_id'):
            $state = DB::table('states_translations')
                    ->select('states_name')
                    ->where('states_id', $request->changedData)
                    ->get();
            print json_encode(array('status' => TRUE, 'stateNm' => $state[0]->states_name));
        else:
    ...
    public function getState(Request $request){
        $states = DB::table('states_translations')
                    ->select('states_id', 'states_name')
                    ->where('countries_id', '13')
                    ->orderBy('id', 'ASC')->get()->all();
        $html = '<select class="form-control" id="changedData">';
        if(count($states) > 0):
            foreach($states as $s):
                $selected = ($s->states_id == $request->stateId) ? 'selected' : '';
                $html .= '<option value="'.$s->states_id.'" '.$selected.'>'.$s->states_name.'</option>';
            endforeach;
        endif;
        $html .= '</select>';
        print $html;
