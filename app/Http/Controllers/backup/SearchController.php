...
class SearchController extends Controller
{
  ...
  public function xmlhotelDetails(Request $request, $id){
    ...
    return view('frontend.hotels.xmldetails', compact('dataArray', 'quantity_adults', 'quantity_childs', 'quantity_rooms', 'hotelId', 'regionId', 'keyword', 'totalNight', 't_end', 't_start', 'pageNo'));
  }
  ...
  public function destinationsEnrouteFetchResult(Request $request){
    ...
    $directHotels = array();
    if($fromFishing == 'yes'):
      $dirHotels = Hotels::where('town', '=', $request->keywords)->get()->all();
      if($dirHotels):
    ...
    if($request->keywords && $request->region_id != ''):
        ...
        $pageNo     = $request->pageNo;
        $take       = ($pageNo * 4);
        $checkAll   = DB::table('hotel_master_xml')->select('id')->where('hotel_master_xml.city_id', $request->region_id)->orWhere('hotel_master_xml.region_id', $postRegionId)->get();
        $check      = DB::table('hotel_master_xml')->where('hotel_master_xml.city_id', $request->region_id)->orWhere('hotel_master_xml.region_id', $postRegionId)->orderBy('stars', 'DESC')->skip(0)->take($take)->get();
    	...
    	if(!$check->isEmpty()):
            ...
            foreach($check as $key => $val):
                ...
                //$hotel      = DB::table('hotel_master_xml')->where('hotel_master_xml.id', $hotelid)->get()->first();
                $hotel      = $val;
                $hotelDesc  = DB::table('hotel_description_xml')->where('hotel_id', $hotelid)->get()->first();
                $images     = DB::table('hotel_images_xml')->select('*')->where('hotel_images_xml.hotel_id', $hotelid)->orderBy('hotel_image_id','ASC')->skip(0)->take(1)->get();
                //$images     = array();
                $amenity    = DB::table('hotel_amenity_xml')->select('Text')->where('hotel_amenity_xml.hotel_id', $hotelid)->orderBy('aminity_id','ASC')->skip(0)->take(5)->get();
                $address    = DB::table('hotel_address_xml')->select('*')->where('hotel_address_xml.hotel_id', $hotelid)->get()->first();
                $hotels[]   = array('hotels' => $hotel, 'images' => $images, 'amenity' => $amenity, 'address' => $address, 'hotelDesc' => $hotelDesc);
            endforeach;
            // echo "<pre>";
            // print_r($hotels); die;
    ...
    print json_encode(array(
      ...
      "html"            => $html,
      "count"           => $count,
      "totalHotelList"  => json_encode($hotels),
    ...
    public function destinationsEnrouteFetchResultLoadMore(Request $request){
        ...
        if(!empty($pageNo)):
            $skip           = ($pageNo * 4);
            $check          = DB::table('hotel_master_xml')->select('id')->where('hotel_master_xml.city_id', $postRegionId)->orWhere('hotel_master_xml.region_id', $postRegionId)->orderBy('stars', 'DESC')->skip($skip)->take(4)->get();
        endif;
        if(!$check->isEmpty()):
            $pageNo         += 1;
            $success        = TRUE;
            foreach($check as $key => $val):
                ...
                //$amenity    = DB::table('hotel_amenity_xml')->select('Text')->where('hotel_amenity_xml.hotel_id', $hotelid)->get();
                $images     = DB::table('hotel_images_xml')->select('*')->where('hotel_images_xml.hotel_id', $hotelid)->orderBy('hotel_image_id','ASC')->skip(0)->take(1)->get();
                $amenity    = DB::table('hotel_amenity_xml')->select('Text')->where('hotel_amenity_xml.hotel_id', $hotelid)->orderBy('aminity_id','RAND')->skip(0)->take(5)->get();
                $address    = DB::table('hotel_address_xml')->select('*')->where('hotel_address_xml.hotel_id', $hotelid)->get()->first();
                $hotels[]   = array('hotels' => $hotel, 'images' => $images, 'amenity' => $amenity, 'address' => $address, 'hotelDesc' => $hotelDesc);
            endforeach;
            $html           = view('frontend.hotels.ajaxNewHotel', compact('hotels', 'currency', 'result', 'price', 'quantity_adults', 'quantity_childs', 'quantity_rooms', 't_start', 't_end', 'totalNight', 'keyword', 'pageNo'))->render();
