              ...
              <div class="header_search_area">
                <div class="form_box_large search_box_comman">
                  <div class="header_search">
                    <input type="text" autocomplete="off" class="header_search_txt" name="keywords" id="enkeywords" placeholder="Search.." value="<?php echo isset($_GET['keywords']) ? $_GET['keywords'] : ''; ?>">
                ...
                <div class="form_box_small search_box_comman room_area">
                  ...
                  <div class="form-group">
                    <div class="input-room-row" id="input_room_row">
                      ...
                      $rm=0;
                      $gest=0;
                      if(isset(session('ab')['norm'])) { 
                        foreach(session('ab')['norm'] as $ka=>$abk){ 
                          $rm = $rm+session('ab')['norm'][$ka];
                          $gest = $gest+((session('ab')['adlts'][$ka]>0?session('ab')['adlts'][$ka]:0)+ (session('ab')['kids'][$ka]>0?session('ab')['kids'][$ka]:0));
                        }
                      }else{
                        $rm=1;
                        $gest=1;
                    ...
                    <div class="room-container" id="room_container"  style="display:none">
                      ...
                      <?php
                      if(isset(session('ab')['norm'])) { 
                        foreach(session('ab')['norm'] as $k=>$ab){ $a = $k;?>
                          <div class="room-rows">
                            <input type="hidden" id="norm" name="ab[norm][]" value="<?php echo session('ab')['norm'][$k];?>" />
                            <div class="room-cell">
                              <span class="roomCount displayBlock">Room <?php echo ++$a; ?></span>
                            </div>
                            <div class="room-cell">
                              <span class="guestminus">
                                <select name="ab[adlts][]" class="adlts" onchange="update_rmaudkik();">
                                  ...
                                  for($i = 1; $i <= $max_adults_search; $i++){
                                    if($i==1){
                                      echo '<option value="'.$i.'" '.( session('ab')['adlts'][$k]==$i?'selected="selected"':'').' >'.$i.' Adult </option>';  
                                    }else{
                                      echo '<option value="'.$i.'" '.( session('ab')['adlts'][$k]==$i?'selected="selected"':'').'>'.$i.' Adults </option>';
                                    }
                                  } ?>  
                                </select>
                              </span>
                              <span class="guestplus">
                                <select name="ab[kids][]" class="kids" onchange="update_rmaudkik();">
                                  ...
                                  for($i = 1; $i <= $max_children_search; $i++){
                                    if($i==1){
                                      echo '<option value="'.$i.'" '.( session('ab')['kids'][$k]==$i?'selected="selected"':'').' >'.$i.' Kid </option>';  
                                    }else{
                                      echo '<option value="'.$i.'" '.( session('ab')['kids'][$k]==$i?'selected="selected"':'').' >'.$i.' Kids </option>';
                                    }
                                  } ?> 
                                </select>
                              </span>
                              <?php echo ($a==1?'<span class="remove_non"></span>':'<span class="remove" onclick="removeRow(this)">Remove</span>'); ?>
                            </div>
                          </div>
                        <?php } 
                      }else{ ?>
