      ...
      <div class="col-md-12">
        <div class="card">
          <div class="card-header card-header-primary">
            <h3 class="card-title">Edit Amenities
              <div class="float-right">
                <select id="sitelang" name="sitelang" class="browser-default btn-round custom-select">
                  <?php @langOption(); ?>
                </select>
                <a href="{{ route('admin.amenities')}}" class="btn-sm btn-success btn-round ">
            ...
            <div class="card-body">
              ...
              <?php 
                $lang = @\Session::get('language');
                 ?>
              <form id="EditAmenities" method="post" action="{{ route('admin.amenities.update', ['lang' => $lang, 'id' => $amenities->id]) }}" enctype="multipart/form-data">
                {{ csrf_field() }}
                <input type="hidden" id="lang_code" name="locale" value="en">
                <div class="row">
                  <div class="col-md-10">
                    <div class="form-group">
                      <label class="bmd-label-floating">Amenities Name</label>
                      <input type="text" id="amenities_name" name="amenities_name" class="form-control" value="{{ $amenities->amenities_name }}">
                    </div>

                    <div class="form-group">
                      <label class="bmd-label-floating">Amenities Status</label>
                      <select id="status" name="status" class="form-control">
                        <option value="1" @if($amenities->status == 1) selected @endif>Published</option>
                        <option value="2" @if($amenities->status == 2) selected @endif>Draft</option>
                      </select>
                    </div>
                  </div>
                  <div class="col-md-2">
