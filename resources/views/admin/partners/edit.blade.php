      ...
      <div class="col-md-12">
        <div class="card">
          <div class="card-header card-header-primary">
            <h3 class="card-title">Edit Partner
              <div class="float-right">
                <select id="sitelang" name="sitelang" class="browser-default btn-round custom-select">
                  <?php @langOption(); ?>
                </select>
                <a href="{{ route('admin.partners')}}" class="btn-sm btn-success btn-round ">
            ...
            <div class="card-body">
              ...
              <form id="EditPartner" method="post" action="{{ route('admin.partners.update', ['id' => $partner->id]) }}" enctype="multipart/form-data">
                {{ csrf_field() }}
                <input type="hidden" id="lang_code" name="locale" value="en">
                <div class="row">
                  <div class="col-md-10">
                    <div class="form-group">
                      <label class="bmd-label-floating">Partner Title<sup style="color:red">*</sup> </label>
                      <input type="text" id="partner_title" name="partner_title" class="form-control" value="{{ $partner->title }}">
                    </div>
                     <div class="form-group">
                      <label class="bmd-label-floating">Partner URL</label>
                      <input type="text" id="link" name="link" class="form-control" value="{{ $partner->link }}">
                    </div>
                    <div class="form-group">
                      <label class="bmd-label-floating">Partner Image<sup style="color:red">*</sup></label>
                      <div class="file-field">
                        <div class="z-depth-1-half mb-4">
                          @if($partner->image)
                          <img src="{{ Storage::disk('local')->url($partner->image) }}" class="img-fluid" alt="example placeholder" width="150px" height="150px">
                          @endif
                        </div>
                        <div class="d-flex">
                  ...
                  <div class="col-md-2">
                    <div class="form-group float-right">
                      <label class="bmd-label-floating">Partner Status</label>
                      <select id="status" name="status" class="form-control">
                        <option value="1" @if($partner->status == 1) selected @endif>Published</option>
                        <option value="2" @if($partner->status == 2) selected @endif>Draft</option>
                      </select>
                    </div>
                  </div>
                </div>
