      ...
      <div class="col-md-12">
        <div class="card">
          <div class="card-header card-header-primary">
            <h3 class="card-title">Edit State
          ...
          <div class="card-body">
            ...
            ?>
            <form id="EditState" method="post" action="{{ route('admin.states.update', ['lang' => $lang, 'id' => $state->id]) }}">
              {{ csrf_field() }}
              ...
              <div class="row">
                <div class="col-md-10">
                  <div class="form-group">
                    ...
                    <input type="text" id="states_name" name="states_name" class="form-control" value="{{ $state->states_name }}">
                  </div>
                  <div class="form-group">
                    <label class="bmd-label-floating">Country</label>
                    <select id="countries_id" class="form-control" name="countries_id">
                      <option>---Please Select---</option>
                      <?php @countryOption($state->countries_id); ?>
                    </select>
                  </div>
                </div>
                <div class="col-md-2">
                  <div class="form-group float-right">
                    <label class="bmd-label-floating">State Status</label>
                     <select id="status" name="status" class="form-control">
                      <option value="1" @if($state->status == 1) selected @endif>Published</option>
                      <option value="2" @if($state->status == 2) selected @endif>Draft</option>
                    </select>
                  </div>
                </div>
              </div>
