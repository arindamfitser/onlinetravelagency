      ...
      <div class="col-md-12">
        <div class="card">
          <div class="card-header card-header-primary">
            <h3 class="card-title">Edit Country
            <div class="float-right">
              <select id="sitelang" name="sitelang" class="browser-default btn-round custom-select">
                <?php @langOption(); ?>
              </select>
              <a href="{{ route('admin.countries')}}" class="btn-sm btn-success btn-round ">
          ...
          <div class="card-body">
            ...
            ?>
            <form id="EditCountry" method="post" action="{{ route('admin.countries.update', ['lang' => $lang, 'id' => $country->id]) }}">
              {{ csrf_field() }}
              <input type="hidden" id="lang_code" name="locale" value="{{ $country->locale }}">
              <div class="row">
                <div class="col-md-10">
                  <div class="form-group">
                    <label class="bmd-label-floating">Country Name</label>
                    <input type="text" id="countries_name" name="countries_name" class="form-control" value="{{ $country->countries_name }}">
                  </div>
                  <div class="form-group">
                    <label class="bmd-label-floating">Country Sortcode</label>
                    <input type="text" id="countries_sortname" name="countries_sortname" class="form-control" value="{{ $country->countries_sortname }}">
                  </div>
                  <div class="form-group">
                    <label class="bmd-label-floating">Phonecode</label>
                    <input type="text" id="countries_phonecode" name="countries_phonecode" class="form-control" value="{{ $country->countries_phonecode }}">
                  </div>
                </div>
                <div class="col-md-2">
                  <div class="form-group float-right">
                    <label class="bmd-label-floating">Country Status</label>
                    <select id="status" name="status" class="form-control">
                      <option value="1" @if($country->status == 1) selected @endif>Published</option>
                      <option value="2" @if($country->status == 2) selected @endif>Draft</option>
                    </select>
                  </div>
                </div>
              </div>
